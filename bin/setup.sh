...
chmod +x ./bin/* || { echo -e $(date -u) "\t[Error] giving exec permission to bin/" ; exit 1; }
chmod +x ./viewer/bin/* || { echo -e $(date -u) "\t[Error] giving exec permission to bin/" ; exit 1; }

# Install dependencies: nginx 1.17.8 postgresql 12.3 ruby 2.6.5
echo -e $(date -u) "\t[Installing pkg dependencies]"
pkg update -y || { echo -e $(date -u) "\t[Error] pkg update" ; exit 1; }
pkg install nginx postgresql ruby debianutils -y || { echo -e $(date -u) "\t[Error] pkg installs" ; exit 1; }

# Adds nginx config
ln -sf $WD/config/nginx.conf $PREFIX/etc/nginx/nginx.conf

echo -e $(date -u) "\t[Installing Ruby gem dependencies]"
gem install bundler || { echo -e $(date -u) "\t[Error] gem install bundler" ; exit 1; }
BUNDLE=`which bundle`
...
mkdir -p log
$WD/bin/restart.sh || { echo -e $(date -u) "\t[Error] starting nginx or postgres." ; exit 1; }
if ! pgrep -x "nginx" > /dev/null
then
  echo -e $(date -u) "\t[Error] starting nginx. If you don't have SSL certs, remove ssl-related settings from config/nginx.conf (listen 8443 ssl; ssl_certificate and ssl_certificate_key)." ; exit 1;
else
  nginx -s reload
fi
...
psql -U $PSQL_SUPERUSER postgres -tAc "SELECT 1 FROM pg_roles WHERE rolname='$DB_USERNAME'" | grep -q 1 \
 ...
 || { echo -e $(date -u) "\t[Error] creating db user" ; exit 1; }
...
DISABLE_DATABASE_ENVIRONMENT_CHECK=1 RAILS_ENV=production ./viewer/bin/setup || { echo -e $(date -u) "\t[Error] Rails setup" ; exit 1; }

# Adds cron, postgres and nginx init to bash file
echo -e $(date -u) "\t[Adding services to bash_profile]"
cat $WD/bin/restart.sh >> $HOME/.bash_profile || { echo -e $(date -u) "\t[Error] adding services to bash_profile" ; exit 1; }
