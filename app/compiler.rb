...
def write_to_output(string)
  File.open(OUTPUT_FILE, 'a') { |f| f.write(string) }
end

logs = File.read(MAIN_LOG_FILE_PATH)
csv = CSV.new(logs, headers: HEADERS)

return write_to_output("#{Time.now} No logs found.") if logs.size.zero?
...
write_to_output("#{Time.now} Wrote #{csv.count} rows to DB.") if logs.size.zero?
